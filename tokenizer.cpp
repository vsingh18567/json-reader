...
Tokenizer::Tokenizer(std::string s) : s(s) {
    tokenize();
}
...
void Tokenizer::tokenize() {
    ...
    while (idx < s.length()) {
        if (s.substr(idx, 4) == "true") {
            ...
            _tokens.push_back(Token(TokenType::STRING, s.substr(start, end - start), line, col));
            idx = end + 1;
            col++;
        } else if (s[idx] == ':') {
            ...
            _tokens.push_back(Token(TokenType::INT, std::stoi(s.substr(start, end - start)), line, col));
            idx = end;
